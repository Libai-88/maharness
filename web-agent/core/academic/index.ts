    ...
    ctx.register({
      ...
      persona: {
        ...
        content: [
          ...
          '文件名以 list_dir 结果和 get_skill_file 错误信息里的「可用文件」清单为准，禁止凭印象猜测',
          '（如 literature_search_agent.md 这类形似名往往不存在）；读取失败立即用清单里的正确名重试。',
          '长文档（SKILL.md 数万字）：先 get_skill_file 的 grep 参数定位模式/章节（如 grep="outline"），再按需读取，不要把整文塞进上下文。',
